...
def train(iterations=100000, learning_rate=5e-4, n=1):
	sess = tf.Session()
	model = ImageFittingModel(iterations, learning_rate, sess)
	sess.__enter__()
	model.load("all_env/fc_allenv.ckpt")
	...
	for i in range(model.iterations):
		...
		# ind = np.random.choice(1000, 334, replace=False)
		...
		if np.mean(loss) < min_loss:
			...
			# model.save("model/fc_conv2_env0.ckpt")
			model.save("all_env/fc_allenv.ckpt")
...
def evaluate(env):
	...
	while not env.done:
		if env.timesteps % 100 == 0:
			env.save_image("image2_" + str(1000 - env.timesteps) +".png")
