...
def train(iterations=1000000, batchsize=100, path="data/"):
	tf_config = tf.ConfigProto() # fill in w/ custom config 
	tf_config.gpu_options.allow_growth = True
	sess = tf.Session(config=tf_config)
	model = PlannerFittingModel(sess=sess)
	...
	for i in range(iterations):
		...
		a0 = np.load(path+"actions_data_"+ str(ind)+"_0.npy")[ind_list]
		a1 = np.load(path+"actions_data_"+ str(ind)+"_1.npy")[ind_list]
		a2 = np.load(path+"actions_data_"+ str(ind)+"_2.npy")[ind_list]
