...
class DiscreteSingleBnFittingModel:
	def __init__(self, learning_rate=1e-5, sess=tf.Session()):
		...
		self.a0 = tf.placeholder(tf.int32, [None], name="a0")
		...
		with tf.variable_scope("fc3"):
			weights3 = tf.get_variable("weights", [1024, 64], initializer=tf.random_normal_initializer())
			biases3 = tf.get_variable("biases", [64], initializer=tf.constant_initializer(0.0))
			self.logits = tf.matmul(fc2, weights3) + biases3
		self.y = tf.argmax(self.logits,1)

		loss_a0 = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.a0, logits=self.logits)
		# loss_a1 = -tf.reduce_sum(tf.one_hot(tf.reshape(tf.cast(self.a1, tf.int32), [-1]), 64) * tf.log(self.sfm), axis=1)
		# loss_a2 = -tf.reduce_sum(tf.one_hot(tf.reshape(tf.cast(self.a2, tf.int32), [-1]), 64) * tf.log(self.sfm), axis=1)
	...
	def fit(self, state, goal_state, a0):
		_, loss = self.sess.run([self.update_op, self.loss], feed_dict={self.state:state, self.goal_state:goal_state, self.a0:a0})
		return loss

	def evaluate(self, state, goal_state, a0):
		return self.sess.run(self.loss, feed_dict={self.state:state, self.goal_state:goal_state, self.a0:a02})
...
def train(iterations=1000000, batchsize=100, path="param_data/"):
	tf_config = tf.ConfigProto() # fill in w/ custom config 
	tf_config.gpu_options.allow_growth = True
	sess = tf.Session(config=tf_config)
	model = DiscreteSingleBnFittingModel(sess=sess)
	model.sess.__enter__()
	tf.global_variables_initializer().run()
	# model.load("model/discrete_single_bn_model1.ckpt")
	min_loss = 1e2
	for i in range(iterations):
		...
		set_a = random.randint(0, 2)
		a0 = np.load(path+str(set_a)+"discrete_single_bn_actions_data_"+ str(ind)+".npy")[ind_list]

		loss = model.fit(state, goal_state, np.reshape(a0, (batchsize,)))
		# print(loss)
		if loss < min_loss:
			min_loss = loss
			print("Iter "+str(i)+" loss: %.5f" % (loss))
			model.save("model/discrete_single_bn_model1.ckpt")
