...
def train(iterations=1000000, batchsize=128, path="param_data/"):
	...
	model = ParamBnFittingModel(sess=sess)
	...
	for i in range(iterations):
		...
		a0 = np.load(path+"0bounding_circle_normalized_actions_data_"+ str(ind)+".npy")[ind_list]
		a1 = np.load(path+"1bounding_circle_normalized_actions_data_"+ str(ind)+".npy")[ind_list]
		a2 = np.load(path+"2bounding_circle_normalized_actions_data_"+ str(ind)+".npy")[ind_list]
		...
		if loss < min_loss:
			...
			model.save("model/param_bn_model1.ckpt")
