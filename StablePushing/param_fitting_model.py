...
class ParamFittingModel:
	def __init__(self, learning_rate=1e-5, sess=tf.Session()):
		...
		self.a1 = tf.placeholder(tf.float32, [None, 4], name="a1")
		self.a2 = tf.placeholder(tf.float32, [None, 4], name="a2")
...
def train(iterations=1000000, batchsize=100, path="param_data/"):
	...
	model = ParamFittingModel(sess=sess)
	...
	for i in range(iterations):
		...
		a0 = np.load(path+"0bounding_circle_actions_data_"+ str(ind)+".npy")[ind_list]
		a1 = np.load(path+"1bounding_circle_actions_data_"+ str(ind)+".npy")[ind_list]
		a2 = np.load(path+"2bounding_circle_actions_data_"+ str(ind)+".npy")[ind_list]
		...
		if loss < min_loss:
			...
			model.save("model/param_model1.ckpt")
