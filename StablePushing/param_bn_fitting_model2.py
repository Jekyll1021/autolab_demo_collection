...
class ParamBnFittingModel2:
	def __init__(self, learning_rate=1e-5, sess=tf.Session()):
		...
		self.a0 = tf.placeholder(tf.float32, [None, 4], name="a0")
		...
		with tf.variable_scope("fc3"):
			weights3 = tf.get_variable("weights", [1024, 4], initializer=tf.random_normal_initializer())
			biases3 = tf.get_variable("biases", [4], initializer=tf.constant_initializer(0.0))
			self.y = tf.matmul(fc2, weights3) + biases3

		y1, y2 = tf.split(self.y, [2, 2], 1)
		a01, a02 = tf.split(self.a0, [2, 2], 1)
	...
	def evaluate(self, state, goal_state, a0, a1, a2):
		return self.sess.run(self.loss, feed_dict={self.state:state, self.goal_state:goal_state, self.a0:a0})
...
def train(iterations=1000000, batchsize=100, path="param_data/"):
	...
	model = ParamBnFittingModel2(sess=sess)
	model.sess.__enter__()
	tf.global_variables_initializer().run()
	# model.load("/Zisu/model/model1.ckpt")
	min_loss = 1e6	
	for i in range(iterations):
		...
		a0 = np.load(path+str(set_a)+"bounding_circle_normalized_actions_data_"+ str(ind)+".npy")[ind_list]

		loss = model.fit(state, goal_state, a0)
		# print(loss)
		if loss < min_loss:
			min_loss = loss
			
			model.save("model/param_bn_model_nomin1.ckpt")
